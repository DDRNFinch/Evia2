...
return{name,code}}
function patchCourse(){const small=document.querySelector(".evia-app.selfobs .self-top small");if(!small)return;const x=identity();if(!x.name&&!x.code)return;const key=`${x.name}|${x.code}`,nameEl=small.querySelector(".evia-course-name-v110"),codeEl=small.querySelector(".evia-course-code-v110");if(small.dataset.eviaCourseIdentity===key&&nameEl?.textContent===x.name&&codeEl?.textContent===x.code)return;small.dataset.eviaCourseIdentity=key;small.classList.add("evia-course-identity-v110");small.innerHTML=`<span class="evia-course-name-v110"></span><span class="evia-course-code-v110"></span>`;small.querySelector(".evia-course-name-v110").textContent=x.name;small.querySelector(".evia-course-code-v110").textContent=x.code}
