...
function start(){hookStorage();queue();const root=document.getElementById("root")||document.body;if(root&&!root.__eviaEvidenceStateV107){root.__eviaEvidenceStateV107=true;new MutationObserver(records=>{if(relevant(records))queue()}).observe(root,{subtree:true,childList:true})}}
window.addEventListener("load",start);window.addEventListener("pageshow",queue);window.addEventListener("storage",e=>{if([STORE,RPL_KEY,OBS_KEY,WITNESS_KEY].includes(e.key)){queue();setTimeout(queue,0)}});window.addEventListener("evia:milos-observed-changed",()=>{queue();setTimeout(queue,0)});window.addEventListener("evia:witness-changed",()=>{queue();setTimeout(queue,0)});document.addEventListener("click",()=>setTimeout(queue,0),true);if(document.readyState!=="loading")start();else document.addEventListener("DOMContentLoaded",start,{once:true});
